...
//
// - dataflow v2 requires all processes to be typed, and does not support
//   tuple spreading, the pipe operator, or the .out property
//
...
workflow RNASEQ {
    ...
    // quant_ch = QUANT(reads_ch, index: index)    // Channel<id: String, quant: Path>
...
def quantResult(root: Path) -> QuantResult {
    record(
        aux_info:   root.resolve('aux_info/'),
        libParams:  root.resolve('libParams/'),
        logs:       root.resolve('logs/'),
        cmd_info:   root.resolve('cmd_info.json'),
        counts:     root.resolve('lib_format_counts.json'),
        quant:      root.resolve('quant.sf'),
    )
}
